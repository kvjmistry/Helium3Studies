...
#for E in range(1,2):
    ...
    for i in range(start_file, end_file):
        ...
        for t in evt_nums:
            neutrons = part_muon_data.loc[t][part_muon_data.loc[t].name.str.contains('neutron')]
            numofneutrons = len(neutrons)
            #muon = part_muon_data.loc[t][part_muon_data.loc[t].primary]
            #for i in  range(numofneutrons):
                #primarynrgs.append(muon.kin_energy.iloc[0])
            counts=neutrons[neutrons.creator_proc.str.contains('neutronInelastic') ]
            for i in range (1,len(counts)):
                primarynrgs.append(counts.kin_energy.iloc[0])

    xe137_out = savelocat+'neutron_counts_sim'+str(total_sim_events)+'neutrons_f'+str(start_file)+'-'+str(end_file)+'_'+perc+'He3E'+nrg+'.h5'
    pd.DataFrame({'Neutronnrg':primarynrgs}).to_hdf(xe137_out,'nuenergy')